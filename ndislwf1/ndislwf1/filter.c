...
extern IncomingFirewall* incoming_fw;
extern OutgoingFirewall* outgoing_fw;
extern HANDLE incoming_file;
extern HANDLE outgoing_file;
...
--*/
{
    ...
    FChars.ReturnNetBufferListsHandler = /*FilterReturnNetBufferLists*/NULL;
    ...
    if (Status == NDIS_STATUS_SUCCESS)
    {
		...
		// Loading the fw rules
		Status = LoadFWRules(incoming_fw_db_filename, 1); // 1 - Load incoming policy
		Status = LoadFWRules(outgoing_fw_db_filename, 2); // 2 - Load outgoing policy

		Status = ndislwf1RegisterDevice();
		...
		if (Status != NDIS_STATUS_SUCCESS)
		{
			...
			ExFreePoolWithTag(incoming_fw, 'WFSZ');
			ExFreePoolWithTag(outgoing_fw, 'WFSZ');
		}
    }
	else
	{
...
--*/
{
    ...
    DEBUGP(DL_TRACE, "===>ndislwf1 FilterPause: FilterInstance %p\n", FilterModuleContext);
...
--*/
{
	...
	/*if (fw)
	{
		SaveFWRules(incoming_fw_db_filename, 1);
		SaveFWRules(incoming_fw_db_filename, 2);
		ExFreePoolWithTag(fw, 'WFSZ');
	}*/
	if (incoming_fw)
	{
		//SaveFWRules(incoming_fw_db_filename, 1);
	...
	if (outgoing_fw)
	{
		//SaveFWRules(outgoing_fw_db_filename, 2);
		ExFreePoolWithTag(outgoing_fw, 'WFSZ');
	}

	if (incoming_file)
	{
		ZwClose(incoming_file);
	}
	if (outgoing_file)
	{
		ZwClose(outgoing_file);
	}
...
--*/
{
	...
	PNET_BUFFER pCurrentNetBuffer = 0;
	...
	if (pFilter->State == FilterRunning)
	{
		// Going through all net buffer lists
		while (pCurrentNetBufferList)
		{
			// First net buffer of the net buffer list
			pCurrentNetBuffer = NET_BUFFER_LIST_FIRST_NB(pCurrentNetBufferList);

			pNext = NET_BUFFER_LIST_NEXT_NBL(pCurrentNetBufferList);
			NET_BUFFER_LIST_NEXT_NBL(pCurrentNetBufferList) = NULL; /* Unlink */

			// Going through all the net buffer in a net buffer list
			// Supposed to be 1
			while (pCurrentNetBuffer)
			{
				ULONG netbuffer_size = NET_BUFFER_DATA_LENGTH(pCurrentNetBuffer);

				// Get the actual packet from the net buffer
				unsigned char* buffer = (unsigned char*)NdisGetDataBuffer(
					pCurrentNetBuffer,
					netbuffer_size,
					NULL,
					1,
				...
				if (buffer) {
					...
					status = FilterOutgoingPackets(buffer, netbuffer_size);
				}
				else
				{
						
					unsigned char* packet_mdl = MmGetSystemAddressForMdlSafe(NET_BUFFER_FIRST_MDL(pCurrentNetBuffer), NormalPagePriority);
					unsigned int data_offset = NET_BUFFER_DATA_OFFSET(pCurrentNetBuffer);
					unsigned char* packet = packet_mdl + data_offset;
					
					if (packet)
					{
						status = FilterOutgoingPackets(packet, netbuffer_size);
					}
				}
				...
				if (status != STATUS_SUCCESS)
				{
					if (pDropHead)
					{
						NET_BUFFER_LIST_NEXT_NBL(pDropTail) = pCurrentNetBufferList;
						pDropTail = pCurrentNetBufferList;
					}
					else
					{
						pDropHead = pDropTail = pCurrentNetBufferList;
					}
				}
				...
				else
				{
					if (pPassHead)
					{
						NET_BUFFER_LIST_NEXT_NBL(pPassTail) = pCurrentNetBufferList;
						pPassTail = pCurrentNetBufferList;
					}
					else
					{
						pPassHead = pPassTail = pCurrentNetBufferList;
					}
				}
			...
			// next net buffer list
			pCurrentNetBufferList = pNext;
		}

		ASSERT((NetBufferLists == pPassHead) || (NetBufferLists == pDropHead));

		if (pPassHead)
		{
            PNET_BUFFER_LIST temp = pPassHead;
            while (temp->Next != NULL)
            {
                temp = temp->Next;
            }
            DbgPrint("%p\n", pPassHead);
			NdisFSendNetBufferLists(pFilter->FilterHandle, pPassHead, PortNumber, SendFlags);
		}
		if (pDropHead)
		{
			NdisFSendNetBufferListsComplete(pFilter->FilterHandle, pDropHead,
...
#/*if DBG
	...
	while (pCurrentNetBufferList)
	{
		...
		while (pCurrentNetBuffer)
		{
			...
			if (buffer) {
				status = FilterIncomingPackets(buffer, netbuffer_size);
			}
			else
			{
				...
				unsigned char* packet_mdl = MmGetSystemAddressForMdlSafe(NET_BUFFER_FIRST_MDL(pCurrentNetBuffer), NormalPagePriority);
				unsigned int data_offset = NET_BUFFER_DATA_OFFSET(pCurrentNetBuffer);
				unsigned char* packet_data = packet_mdl + data_offset;
				//DbgPrintHexDump(packet_data, 50);

				if (packet_data)
				{
					status = FilterIncomingPackets(packet_data, netbuffer_size);
				}
			}
	...
	ASSERT((NetBufferLists == pPassHead) || (NetBufferLists == pDropHead));
	ASSERT(nDrop + nPass == NumberOfNetBufferLists);

	if (pPassHead)
	{
		NdisFIndicateReceiveNetBufferLists(pFilter->FilterHandle, pPassHead, PortNumber, nPass, ReceiveFlags);
	}
	if (pDropHead)
	{
		NdisFReturnNetBufferLists(pFilter->FilterHandle, pDropHead,
